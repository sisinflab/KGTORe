...
from .KGCNModel import KGCNModel

import numpy as np


class KGCN(RecMixin, BaseRecommenderModel):
    @init_charger
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._params_list = [
            ...
            ("_neighbor_sample_size", "neighbor_sample_size", "neighbor_sample_size", 10, int, None),
            ("_n_iter", "n_iter", "n_iter", 3, int, None),
            ("_batch_eval", "batch_eval", "batch_eval", 128, int, None),
        ...
        kg_graph = sparse.coo_matrix((mapped_relations,
                                      (mapped_subjects, mapped_objects)),
                                     shape=(self._side.n_entities, self._side.n_entities))

        self._model = KGCNModel(
            ...
            l_w=self._l_w,
            aggr=self._aggr,
            neighbor_sample_size=self._neighbor_sample_size,
            kg_graph=kg_graph,
            n_iter=self._n_iter,
    ...
    def get_recommendations(self, k: int = 100):
        ...
        for index, offset in enumerate(range(0, self._num_users, self._batch_eval)):
            ...
            for item_index, item_offset in enumerate(range(0, self._num_items, 8)):
                ...
                p = self._model.predict(torch.tensor(user_range, dtype=torch.int64),
                                        torch.tensor(item_range, dtype=torch.int64),
                                        offset_stop - offset, item_offset_stop - item_offset)
