...
class DecisionPaths:
    ...
    def build_decision_paths(self):
        ...
        with mp.Pool(n_procs) as pool:
            user_decision_paths_max_depth = pool.starmap(user_decision_path, args)

        print("Building decision trees")
        user_decision_paths = [up for up, _ in user_decision_paths_max_depth]

        # save dt depth
        depth = pd.DataFrame(np.array([d for _, d in user_decision_paths_max_depth]))
        depth_name = 'depth' + str(self.npr) + "_" + str(self.criterion) + ".tsv"
        dataset_path = os.path.abspath(os.path.join('./data', self.dataset_name, 'kgtore', depth_name))
        depth.columns = ['private_user_id', 'n_interacions', 'depth']
        depth.to_csv(dataset_path, sep='\t', header=True, index=False)

        self.edge_features = [p for path in user_decision_paths for p in path]
        self.create_edge_features_matrix()

    def tree_depth(self, user: int, clf: DecisionTreeClassifier):
        data = [user, clf.get_depth()]
        # name = 'tree_depth' + str(self.npr) + "_" + str(self.criterion) + ".tsv"
        # dataset_path = os.path.abspath(os.path.join('./data', self.dataset_name, 'kgtore', name))
        self.tree_depth_data.append(data)
...
def user_decision_path(user, user_items, user_i_dict, items: set, item_features: dict, npr, criterion):
    ...
    return u_dp, [user, len(user_i_dict), clf.get_depth()]
