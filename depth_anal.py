...
def depth_analysis(df_dict: dict, nprs: list) -> np.array:
    data = pd.DataFrame(np.array([[int(npr), df_dict[npr]['depth'].mean(), df_dict[npr]['depth'].std()] for npr in nprs]))
    data.columns = ['npr', 'mean_depth', 'std_depth']
...
results = depth_analysis(df_dict=df_dict, nprs=nprs)
print()
