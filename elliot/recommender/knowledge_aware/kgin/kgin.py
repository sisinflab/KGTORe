...
class KGIN(RecMixin, BaseRecommenderModel):
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._params_list = [
            ...
            ("_l2", "l2", "l2", 1e-5, float, None),    # l2 regularization weight
            ("_lfr", "lfr", "lfr", 1e-4, float, None),  # regularization weight for latent factor
            ("_emb", "emb", "emb", 64, int, None),   # embedding size
            ("_hops", "hops", "hops", 3, int, None),  # number of context hops
            ("_n_fac", "n_fac", "n_fac", 4, int, None),    # number of latent factor for user favour
            ("_n_drop", "n_drop", "n_drop", True, bool, None), # consider node dropout or not
            ("_n_drop_r", "n_drop_r", "n_drop_r", 0.5, float, None), # ratio of node dropout
            ("_m_drop", "m_drop", "m_drop", True, bool, None),   # consider message dropout or not
            ("_m_drop_r", "m_drop_r", "m_drop_r", 0.1, float, None), # ratio of node dropout
...
# srotolatore *iterabile = generato degli elementi , **iter = elementi dell'iterabile, ** = dict chiave-valore
        ...
        for h_id, r_id, t_id in tqdm(np.array(self._side.map_), ascii=True):
        ...
        self._model = KGINModel(self._num_users, self._num_items,
                                ...
                                self._hops,
                                self._n_fac,
                                self._n_drop, self._n_drop_r,
                                self._m_drop, self._m_drop_r,
                                self._ind,
                                self._seed)
