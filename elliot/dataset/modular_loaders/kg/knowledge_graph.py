...
class KGRec(AbstractLoader):
    def __init__(self, users: t.Set, items: t.Set, ns: SimpleNamespace, logger: object):
        ...
        self.entity_set = {s for (s, _, _) in self.all_triples} | {o for (_, _, o) in self.all_triples}  # | = union
        self.predicate_set = {p for (_, p, _) in self.all_triples}

        self.nb_examples = len(self.train_triples)

        self.entity_to_idx = {entity: idx for idx, entity in enumerate(sorted(self.entity_set))}  # entity to private
        self.nb_entities = max(
            self.entity_to_idx.values()) + 1  # ha senso farsi sta ricerca ? non basta fare len(self.entity_to_idx) ?
        self.idx_to_entity = {v: k for k, v in self.entity_to_idx.items()}  # private to entity
        ...
        # Triples
        self.Xs, self.Xp, self.Xo = self.triples_to_vectors(self.train_triples, self.entity_to_idx,
    ...
    def triples_to_vectors(self, triples: t.List[t.Tuple[str, str, str]],
        ...
        # return Xs, Xp, Xo
        X = np.array([[entity_to_idx[s], predicate_to_idx[p], entity_to_idx[o]] for (s, p, o) in triples],
                     dtype=np.int32)
        return X[:, 0], X[:, 1], X[:, 2]
