...
class KGTORETSVLoader(AbstractLoader):
    def __init__(self, users: t.Set, items: t.Set, ns: SimpleNamespace, logger: object):
        ...
        self.map_ = self.map_[self.map_['subject'].isin(self.items)]  # solo triple primo hop
        ### solo i 30 predicati più usati:
        predicate_occurencies = set(self.map_['predicate'].value_counts()[:30].index)
        self.map_ = self.map_[self.map_['predicate'].isin(predicate_occurencies)]
        # map2 = self.map_[self.map_['predicate'].isin(predicate_occurencies)]
        # predicate = map2['predicate'].unique()
        #
        ...
        self.items = set.intersection(self.items, self.map_.values[:, 0])
    ...
    def create_namespace(self):
        ns = SimpleNamespace()
        ns.__name__ = "KGTORETSVLoader_backup"
        ns.object = self
