...
class KGZeus(AbstractLoader):
    def __init__(self, users: t.Set, items: t.Set, ns: SimpleNamespace, logger: object):
        ...
        self.nb_entities = max(self.entity_to_idx.values()) + 1  # ha senso farsi sta ricerca ? non basta fare len(self.entity_to_idx) ?
    ...
    def triples_to_vectors(self, triples: t.List[t.Tuple[str, str, str]],
        ...
        ''' non sarebbe meglio:
         X = np.array([[entity_to_idx[s], predicate_to_idx[p], entity_to_idx[o]] for (s,p,o) in triples], dtype=np.int32)
         e poi al più restituire X[:,0] , X[:,1], X[:,2]
         '''
        return Xs, Xp, Xo
